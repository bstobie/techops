...
if ($FSCredentials) {
    ...
    if ($FolderName) {
        ...
        try {
            Write-Host ("`nConnection to file server: [" + $FSSrcShare + "] using credentials: [" + $FSCredentials.UserName + "].`r")
        ...
        foreach ($WebServer in $DestSrvList) {
            ...
            try {
                Write-Host ("`nConnection to web server: [" + $WebSvrShare + "] using credentials: [" + $FSCredentials.UserName + "].`r")
